...
function createTabs(deps) {
  ...
  // 首标签 did-finish-load 后才预热，不与启动首加载抢带宽；认领后后台重建保证下一个新标签同样秒开。
  ...
  function ensureView(rec) {
    ...
    if (claimed) {
      ...
      if (normalizeNotionUrl(wc.getURL()) !== normalizeNotionUrl(rec.url)) {
        ...
        // 兜底：SPA 路由未在 1.5s 内生效（如 Notion 对该 URL 走整页加载）则整页加载
        const target = rec.url;
        setTimeout(() => {
          if (!wc.isDestroyed() && normalizeNotionUrl(wc.getURL()) !== normalizeNotionUrl(target)) {
            wc.loadURL(target);
          }
        }, 1500);
      }
