...
- **单窗口深色非客户区（window-dark-mode.js，跨机器普适性坑）**：消除 Win10 无边框窗口（frame:false）最大化时 DWM 残留的 1px 白边——对该窗口 HWND 调 `DwmSetWindowAttribute(DWMWA_USE_IMMERSIVE_DARK_MODE)`，不动系统主题、不影响其他程序。**属性号跨版本变过**：Win10 2004（build 19041+）为 20，更早版本（1909 等）为 19；脚本必须**先试 20、返回非 0 再退 19**，只发 20 在旧 build 机器上会静默失效。返回值**不能** `| Out-Null` 吞掉：脚本回写一行 `nd-dwm ok/fail ...`（失败带两个 HRESULT，顺带回读 `ColorPrevalence`——用户开了"在窗口边框显示强调色"时边框走系统强调色会盖过本属性），主进程按 `log`（index.js 的 `dwmLog`，`ND_PERF=1` 时输出 `[dwm]` 行）回报，失败不再静默。调用点：index.js 窗口创建后一次 + themeService.onApplied 每次**有效上报**（含主题未变的重设，见上条）。**真正的跨机器白线根因不在属性号**，而在 theme-service 的重设时机——同 build(19045) 机器一台生效一台不生效，是持久化 dark 时早期 NC 调用因窗口未就绪未生效、而 Notion 上报 dark 又被"主题未变"去重不重设所致（v0.2.7 修）。**又一层跨机器坑（v0.2.8 实测，最终改用 koffi）**：`powershell.exe`（Windows PowerShell 5.1）在部分机器上跑 `Add-Type` 编译 C# 会失败——不是闪退，而是编译错误（受限语言模式 CLM / 缺 csc.exe / 安全软件拦截），`[DwmApi]` 类型不存在，`DwmSetWindowAttribute` 抛「找不到类型」非终止错误、返回值留空（日志 `r20= r19=` 空值），且脚本仍打印 `nd-dwm fail` 状态行，导致 pwsh 回退不触发。**最终方案：改用 koffi 原生 FFI（N-API 预编译，不依赖 .NET 编译器、不受 CLM 限制）直接从主进程调 `DwmSetWindowAttribute`**，PowerShell 链路降级为 koffi 加载失败时的回退。koffi 是 `dependencies`，原生二进制在 `@koromix/koffi-win32-x64`（optionalDependency），`asarUnpack` 必须含 `node_modules/@koromix/**`（否则 .node 在 asar 里加载失败）。**HWND 传参坑（实测）**：`getNativeWindowHandle()` 返回的是「存有 HWND 值的 Buffer」，koffi 的 `void*` 参数要的是指针值本身——必须 `readBigInt64LE(0)` 读 BigInt 再 `koffi.as(n, 'void*')` 转指针，直接传 Buffer 会传成 Buffer 地址（返回 E_HANDLE）。改 koffi 路径必须测 `tests/window-dark-mode.test.js` 的 koffi 用例（含 toPtr 转换）。
