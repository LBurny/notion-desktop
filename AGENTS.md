...
- **设置子窗口随页面缩放**：样式/设置窗的宽高都要按 `settingsWindowSize(340, 基准高, zoom, 工作区上限)` 等比放大并 setZoomFactor（只放高度会横向裁剪），打开时和 `applyZoomEverywhere` 里都要同步。新增表单行时同步加大 configs 基准高度（样式页 380 起）。窗口关闭是**隐藏缓存**（settings-windows.js 拦截 close + hide），重开即显；应用退出（isQuitting）才真正销毁。窗口为 `transparent: true`（不再设 backgroundColor），圆角由页面 body 的 border-radius: 4px 绘制（与托盘菜单同款）。
