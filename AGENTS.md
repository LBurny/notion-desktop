...
- **附件/文件点击不得开标签（v0.2.10 实测）**：点附件是 `window.open('https://www.notion.so/signed/attachment:<uuid>:<文件名>?table=block&id=…')`——URL 以 notion.so 开头但 302 到 file.notion.so 且 `Content-Disposition: attachment`，是下载不是页面；旧逻辑按前缀判成页面开了新标签（标签内容是认领的预热视图残留，看起来随机）。window-open 分流归口 tab-manager.js 的纯函数 `classifyWindowOpen`（auth-popup / download / notion-page / external 四类，顺序敏感：`/signed/` 是 www.notion.so 子路径，必须先于页面规则判断；tests/tab-manager.test.js 看护），download 类在 tabs.js 走 `wc.session.downloadURL(url)` 原地下载。保存框要等 302 响应头到达才弹，网络往返属浏览器同款延迟，非套壳引入。
...
- **分区字体选择器必须镜像旧全集（实测根因）**：users 的 custom.css 是旧版 default.css 的副本（排在 default.css 与设置 CSS 之后）。正文文字叶节点由 `[contenteditable="true"]:first-of-type` 这类 **(0,2,0)** 规则命中，而 `.notion-page-content *` 只有 (0,1,0)（`*` 不贡献特异性）——设置字体规则压不过旧副本，表现是「改正文字体只有目录/条目编号等容器变了，真正文字没变」（cdp-fonts-check 带真实叶节点断言防此类假绿）。所以 style-settings.js 的 BODY_SELECTORS/UI_SELECTORS 必须镜像旧版 default.css 全局字体表的完整选择器（同特异性 + 设置 CSS 最后注入 → 稳定覆盖）；想简化选择器前先看这条。**代码块/公式/行内代码正文同款坑（实测，CDP 验证）**：代码正文、行内公式、行内代码都在 `div[contenteditable="true"]` 内，被正文 `!important` 规则压过——`CODE_SELECTORS`/`MATH_SELECTORS` 若只到 `.notion-code-block *` (0,1,0)，代码体/行内代码会落到正文字体（仅语言标签等非 contenteditable 部分生效）。`CODE_SELECTORS` 需镜像 `.notion-code-block div[contenteditable="true"] *` (0,2,1) 覆盖块代码正文，`.notion-page-content .notion-inline-code-container *` (0,2,0) 覆盖行内代码；`MATH_SELECTORS` 行内用 `.katex:not(.katex-display .katex)` 高特异性覆盖。**行内代码真实结构**：Notion 行内代码是 `div.notion-inline-code-container`（**不是 `<code>`，页面里 0 个 code 元素**），内部 `span.notion-enable-hover` 带**内联 `font-family`（等宽，无 !important）**——正文 `!important` 规则压过内联样式，故须用 `!important` 代码规则夺回；行内代码不在 `.notion-page-block` 内（正文最高只到 `.notion-page-content *` 0,1,0 与 `:first-of-type` 0,2,0）。`customFontChain` 已对回退栈去重（`normalizeFontName`），避免用户字体与栈首项重名（如 code=Consolas 与 CODE_FONT_STACK 首项 Consolas）。cdp-fonts-check 的代码探针已镜像真实结构（块代码 contenteditable 嵌套 + 行内代码 `.notion-inline-code-container` 内联 span），并校验页面真实行内代码，改代码/公式选择器后跑该脚本即可防回归。
...
- **主题探测与持久化**：Notion 账户主题要等 JS 就绪后才打 dark class，启动早期探测恒为 light——持久化（theme.json，theme-store.js）优先于 `nativeTheme.shouldUseDarkColors`（混合模式系统拿到的是浅色）；启动 15s 宽限期内忽略与持久化 dark 冲突的 light 上报，否则 theme.json 被假象污染。加载期白闪双保险：视图 `backgroundColor` + content.js 的 document-start 早期 `<style>html{background:#191919}`（sendSync get-theme 取值，主题反转时摘除）。**`report()` 收到有效主题即便与当前相同也触发 `onApplied`（只在变化时落盘）**：持久化 dark 时启动早期那次 NC 设置可能因窗口尚未就绪而未生效，Notion 上报 dark（窗口已就绪）时需要这次重设机会——否则 Win10 无边框窗口 1px 白边会一直残留（跨机器时序差异根因，曾误判为 DWM 属性号问题）。
- **单窗口深色非客户区（window-dark-mode.js，跨机器普适性坑）**：消除 Win10 无边框窗口（frame:false）最大化时 DWM 残留的 1px 白边——对窗口 HWND 调 `DwmSetWindowAttribute(DWMWA_USE_IMMERSIVE_DARK_MODE)`，不动系统主题、不影响其他程序。**属性号跨版本变过**：Win10 2004（build 19041+）为 20，更早版本（1909 等）为 19，**先试 20、返回非 0 再退 19**，只发 20 在旧 build 机器上会静默失效。**实现用 koffi 原生 FFI（N-API 预编译）直调**，不走 PowerShell——曾用 `Add-Type` 内联编译 C#，但部分机器上编译失败（受限语言模式 CLM / 缺 csc.exe / 安全软件拦截），`[DwmApi]` 类型不存在、调用抛「找不到类型」非终止错误、返回值留空，白边残留（v0.2.8 弃用）。koffi 是 `dependencies`，原生二进制在 `@koromix/koffi-win32-x64`（optionalDependency），`asarUnpack` 必须含 `node_modules/@koromix/**`（否则 .node 在 asar 里加载失败）。**HWND 传参坑（实测）**：`getNativeWindowHandle()` 返回「存有 HWND 值的 Buffer」，koffi 的 `void*` 参数要指针值本身——`readBigInt64LE(0)` 读 BigInt 再 `koffi.as(n, 'void*')` 转指针，直接传 Buffer 会传成 Buffer 地址（返回 E_HANDLE）。调用点：index.js 窗口创建后一次 + themeService.onApplied 每次**有效上报**（含主题未变的重设）——同 build(19045) 机器一台生效一台不生效的真因曾是 theme-service 重设时机（持久化 dark 时早期 NC 调用因窗口未就绪未生效，Notion 上报 dark 又被"主题未变"去重不重设，v0.2.7 修）。改 koffi 路径必须测 `tests/window-dark-mode.test.js` 的 koffi 用例（含 toPtr 转换）。
